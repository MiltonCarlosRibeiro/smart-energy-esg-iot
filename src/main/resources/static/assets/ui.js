window.UI = (function () {
  ...
  function bindButtons() {
    document.getElementById("btnPresence").onclick = () => {
    ...
    document.getElementById("btnSend").onclick = async () => {
      try {
        await window.IOT.sendTelemetry({ ...state });
        // atualiza imediatamente depois de enviar
        const latest = await window.IOT.fetchLatest(state.room);
        updateStatusView(latest);
      } catch (e) {
        console.error("Falha ao enviar telemetria:", e);
      }
    };
  }
